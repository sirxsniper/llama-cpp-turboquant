...
static void ggml_vec_dot_turbo5p_0_f32(int n, float * GGML_RESTRICT s, size_t bs,
                                       ...
                                       const void * GGML_RESTRICT vy, size_t by, int nrc);
// [TAG_TURBO5P512]
static void ggml_vec_dot_turbo5p512_0_f32(int n, float * GGML_RESTRICT s, size_t bs,
                                          const void * GGML_RESTRICT vx, size_t bx,
                                          const void * GGML_RESTRICT vy, size_t by, int nrc);
...
static const struct ggml_type_traits_cpu type_traits_cpu[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_TURBO5P512_0] = {
        .from_float               = (ggml_from_float_t) quantize_row_turbo5p512_0_ref,
        .vec_dot                  = (ggml_vec_dot_t) ggml_vec_dot_turbo5p512_0_f32,
        .vec_dot_type             = GGML_TYPE_F32,
        .nrows                    = 1,
    },
};
...
// [TAG_MOE_LOG_PORTABLE] flockfile/funlockfile are POSIX; MSVC spells them _lock_file/_unlock_file.
// This only guards the optional expert-routing debug log, so a no-op fallback is also fine.
static inline void ggml_moe_log_lock(FILE * f) {
#if defined(_MSC_VER)
    _lock_file(f);
#elif defined(_WIN32)
    (void) f;
...
static inline void ggml_moe_log_unlock(FILE * f) {
#if defined(_MSC_VER)
    _unlock_file(f);
#elif defined(_WIN32)
    (void) f;
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        if (!ggml_cpu_extra_work_size(n_threads, node, &cur)) {
            switch (node->op) {
                ...
                case GGML_OP_FLASH_ATTN_EXT:
                    {
                        ...
                        // [TAG_CPU_FA_DV_PAD] the tiled kernel lays VKQ32 and V32 out with row stride DVp (ops.cpp)
                        const int64_t DVp = ggml_fa_tiled_dv_pad(DV);

                        // Tiled flash attention scratch (tile sizes defined in common.h)
                        // Per-thread: Q_q + KQ + mask + VKQ32 + V32 + K_f32 + padding
                        size_t prefill  = sizeof(float)*(GGML_FA_TILE_Q*DK + 2*GGML_FA_TILE_Q*GGML_FA_TILE_KV + GGML_FA_TILE_Q*DVp + GGML_FA_TILE_KV*DVp + GGML_FA_TILE_KV*DK)*n_tasks;
                ...
                case GGML_OP_GATED_DELTA_NET_REPLAY: // [TAG_4C_GDN_REPLAY] delta and a working state per thread
                    {
                        const int64_t S_v = node->src[2]->ne[0];
...
static void ggml_vec_dot_turbo5p512_0_f32(int n, float * GGML_RESTRICT s, size_t bs,
    ...
    float tmp[4096];
    GGML_ASSERT(n <= 4096);
    GGML_ASSERT(n % QK_TURBO5P512 == 0);
    ggml_get_type_traits(GGML_TYPE_TURBO5P512_0)->to_float(vx, tmp, n);
