...
bool ggml_cuda_should_use_mmvq(enum ggml_type type, int cc, int64_t ne11, int64_t ne01) {
    ...
    if (GGML_CUDA_CC_IS_NVIDIA(cc) && cc == GGML_CUDA_CC_BLACKWELL) {
        ...
        switch (type) { // tuned on RTX 5090
            ...
            case GGML_TYPE_Q3_K:
            case GGML_TYPE_Q4_K:
            // [TAG_SYNC_MMVQ_Q5K] upstream moved Q5_K to ne11 <= 6 on Blackwell; kept at 5 with Q2..Q4_K (only
            // ne11 == 6 differs, and the ceilings above were measured to hold). GGML_CUDA_MMVQ_MAX_K overrides for A/B.
            case GGML_TYPE_Q5_K:
