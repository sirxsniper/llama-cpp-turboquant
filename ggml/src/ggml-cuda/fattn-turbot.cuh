...
#pragma unroll
        for (int col = 0; col < cols_per_thread; ++col) {
            ...
            // [TAG_FA_SINK_CLAMP] a padded head column (zt_gqa*ncols2 + jc % ncols2 >= gqa_ratio, gqa 6 in ncols2 8)
            // reads the last real sink of its group instead of past the end of sinks. Its output is never written, and
            // every real column reads the same sink as before.
