// has_ids is carried through every mmq config for a uniform dispatch signature.
// The Pascal tables do not branch on it - the has_ids split was only ever measured
// and tuned on Blackwell - but taking the parameter keeps ggml_cuda_mmq_get_config
// able to call any backend config the same way.
static constexpr __host__ __device__ ggml_cuda_mmq_config ggml_cuda_mmq_get_config_pascal_dp4a(ggml_type type, int J, bool fallback, bool has_ids) {
