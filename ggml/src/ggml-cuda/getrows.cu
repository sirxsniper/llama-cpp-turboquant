...
template <typename dst_t>
static __global__ void k_get_rows_turbo5p512(
        const void * __restrict__ src0, const int32_t * __restrict__ src1, dst_t * __restrict__ dst,
        const int64_t ne00, const int64_t nz, const uint3 ne12_fdv,
        const size_t s1, const size_t s2, const size_t s3,
        const size_t nb01, const size_t nb02, const size_t nb03,
    ...
    for (int64_t z = blockIdx.z; z < nz; z += gridDim.z) {
        ...
        // Warp-uniform loop bound, so a whole warp always runs the same iterations and the
        // full-mask __shfl_sync inside turbo5p512_dequant_lane stays legal.
        for (int64_t iw = (int64_t) blockIdx.y * warps_per_block + warp_in_block;
             iw < warps_per_row;
             iw += (int64_t) gridDim.y * warps_per_block) {
            ...
            turbo5p512_dequant_lane<dst_t>((const block_turbo5p512_0 *) xrow + ib, jb,
                                        raw_centroid, yrow + j);
        }
    }
}
...
template<typename dst_t>
static void get_rows_cuda_turbo5p512(
        const void * src0_d, const int32_t * src1_d, dst_t * dst_d,
        const int64_t ne00, const size_t nb01, const size_t nb02, const size_t nb03,
        const int64_t ne10, const int64_t ne11, const int64_t ne12,
        const size_t nb10, const size_t nb11, const size_t nb12,
        const size_t nb1, const size_t nb2, const size_t nb3,
        cudaStream_t stream) {
    GGML_ASSERT(ne00 % QK_TURBO5P512 == 0 && "turbo5p get_rows needs block-aligned rows");
    ...
    k_get_rows_turbo5p512<dst_t><<<block_nums, block_dims, 0, stream>>>(
        src0_d, src1_d, dst_d, ne00, ne11*ne12, ne12_fdv,
        s1, s2, s3, nb01, nb02, nb03, s10, s11, s12);
}
...
static void ggml_cuda_get_rows_switch_src0_type(
    ...
    switch (src0_type) {
        ...
        case GGML_TYPE_TURBO5P512_0:
            get_rows_cuda_turbo5p512(src0_d, src1_d, dst_d,
                ne00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb1, nb2, nb3, stream);
            break;
