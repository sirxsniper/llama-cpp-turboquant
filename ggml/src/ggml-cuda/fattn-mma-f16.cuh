...
static constexpr __host__ __device__ fattn_mma_config ggml_cuda_fattn_mma_get_config_ampere(const int DKQ, const int DV, const int ncols) {
    ...
    // nbatch_fa 32 -> 64 for this shape: measured pp512 1204.33 -> 1230.04 at d131072
...
// full as F16 in global scratch (fattn-common.cuh, to_fp16 over ggml_nelements(K)) once
// per layer per token. That scratch is ~4x the bytes of the turbo4 cache and is both
// written and read back every call. Measured at d131072: making that conversion free
// (TURBO_SKIP_CONV) lifts the MMA path 35.51 -> 49.17 t/s, and reading turbo4 natively
// also shrinks the tile reads themselves 4x on top of that.
//
// cp_async cannot be used here - it is a straight global->shared DMA with no ALU on the
// way - so this is the synchronous path. That is affordable because the tile is 4x
// smaller in global memory than its f16 equivalent.
//
// Layout note: a turbo4 row of D elements is D/128 blocks of 68 bytes. `elem0` and the
// per-chunk step are both multiples of 8, so `qs + (j0>>1)` is always 4-byte aligned and
// the nibble pair load is a single LDG.E.32 (this is exactly what the `pad` field in
// block_turbo4_0 exists to guarantee).
...
static __device__ __forceinline__ void flash_attn_ext_turbo4_load_tile(
        const char * const __restrict__ KV,      // first row of the tile, turbo4 bytes
        half2 * const __restrict__ tile_KV,
        const int D2,                             // half2 per row to produce
        const int stride_KV_bytes,                // row stride in bytes
        const int elem0,                          // first element within the row
        const int i_sup) {
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();

    // 8 elements = 4 half2 = 16 bytes written per chunk, matching the f16 loader.
...
#pragma unroll
                    ...
                    const block_turbo4_0 * __restrict__ blk =
                        (const block_turbo4_0 *) (KV + (size_t) i * stride_KV_bytes) + (e0 / QK_TURBO4);
                    const int j0 = e0 % QK_TURBO4;

                    uint32_t qs4;
                    ggml_cuda_memcpy_1<4, 4>(&qs4, blk->qs + (j0 >> 1));
                    const float norm = __half2float(blk->norm);

#pragma unroll
                    for (int e = 0; e < h2_per_chunk; ++e) {
                        const unsigned i0n = (qs4 >> (8*e + 0)) & 0xFu;
                        const unsigned i1n = (qs4 >> (8*e + 4)) & 0xFu;
                        out[e] = make_half2(TURBO_CENTROIDS_4BIT[i0n] * norm,
                                            TURBO_CENTROIDS_4BIT[i1n] * norm);
                    }
                }
...
template<int stride_tile, int nwarps, int nbatch_fa, bool oob_check>
static __device__ __forceinline__ void flash_attn_ext_turbo5p512_load_tile(
        const char * const __restrict__ KV,      // block base of the tile's first KV position
        half2 * const __restrict__ tile_KV,
        const int D2,                             // half2 per row to produce, must be a multiple of 16
        const int stride_KV_bytes,                // KV position stride in bytes
...
#pragma unroll
                    ...
                    const int e0 = elem0 + k*elem_per_chunk;
                    const block_turbo5p512_0 * __restrict__ blk =
                        (const block_turbo5p512_0 *) (KV + (size_t) i * stride_KV_bytes) + (e0 / QK_TURBO5P512);
                    const int j0 = e0 % QK_TURBO5P512;
                    ...
                    __align__(16) uint32_t qs[4];
                    ggml_cuda_memcpy_1<16>(qs, blk->qs + (j0 >> 1));
                    // [TAG_TURBO5P512] 32 high bits at qh[j0/8]; j0 is a multiple of 32, one aligned 4-byte load.
                    uint32_t qh;
                    ggml_cuda_memcpy_1<4>(&qh, blk->qh + (j0 >> 3));
                    ...
                    turbo5_int8_lut lut;
                    lut.init();
                    const float nscale = __half2float(blk->norm[j0 / QK_TURBO5P512_GROUP]) * TURBO_INT8_5BIT_SCALE_REVERSE;
...
// [TAG_FA_POS_MASK] Same tile layout as flash_attn_ext_f16_load_mask, but the values come from positions:
// -INF where the cell is empty or later than the query, 0 otherwise. kv_pos is already offset to the tile.
// [TAG_4C_POSMASK_MS] kv_seq (offset to the tile like kv_pos) and q_seq are the sequence sets of a multi-sequence
// positional mask, or nullptr: a cell outside the query's sequence is -INF as well. The same 0 / -INF values the
// explicit mask of llama_kv_cache::set_input_kq_mask holds, so the attention arithmetic is unchanged.
template<int ncols1, int nwarps, int nbatch_fa, bool oob_check>
static __device__ __forceinline__ void flash_attn_ext_f16_gen_mask(
        const int32_t * const __restrict__ kv_pos, const int32_t * const __restrict__ q_pos,
        const int32_t * const __restrict__ kv_seq, const int32_t * const __restrict__ q_seq, half * const __restrict__ tile_mask,
        const int i_sup, const int j0, const uint3 ne01) {
    constexpr int warp_size = ggml_cuda_get_physical_warp_size();
#pragma unroll
    for (int j1 = 0; j1 < ncols1; j1 += nwarps) {
        ...
        const int      qp = q_pos[j_vram];
        const uint32_t qs = kv_seq ? (uint32_t) q_seq[j_vram] : 0u;   // [TAG_4C_POSMASK_MS]
#pragma unroll
        for (int i0 = 0; i0 < nbatch_fa; i0 += warp_size) {
            ...
            if (!oob_check || i < i_sup) {
                ...
                v = (kp < 0 || kp > qp || (kv_seq && ((uint32_t) kv_seq[i] & qs) == 0)) ? -INFINITY : 0.0f;
            }
            tile_mask[j_sram*(nbatch_fa + 8) + i] = __float2half(v);
        }
    }
}

// [TAG_4C_POSMASK_MS] kv_seq advanced to a tile, or nullptr (no pointer arithmetic on a null pointer)
static __device__ __forceinline__ const int32_t * fattn_seq_at(const int32_t * kv_seq, const int k) {
    return kv_seq ? kv_seq + k : nullptr;
}
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps,
    bool use_logit_softcap, bool V_is_K_view, int turbo_KV, bool use_sparse, bool needs_fixup, bool is_fixup, bool last_iter, bool oob_check,
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    ...
    constexpr int  nstages         = turbo_KV ? 0 : ggml_cuda_fattn_mma_get_nstages  (DKQ, DV, ncols1, ncols2, use_sparse);
    // [TAG_SYNC_MMA_SPARSE] upstream's sparse gather exists only in the f16 tile loader; the launcher never pairs it with turbo.
    static_assert(turbo_KV == FATTN_MMA_TURBO_NONE || !use_sparse, "sparse gather is not implemented for turbo KV");

    GGML_UNUSED(turbo_e0);   // only the split-plane layout reads it

    // [TAG_SYNC_TURBO_NOSWZ] The turbo tile loaders write plain rows at i*stride_tile + k*h2, so turbo keeps
    // the unswizzled +4-padded tiles. Must match the shared-memory sizing in ggml_cuda_flash_attn_ext_mma_f16_case.
    constexpr bool swz = turbo_KV == FATTN_MMA_TURBO_NONE && ggml_cuda_fattn_mma_get_swizzled(DKQ, DV, ncols1, ncols2);
    ...
    if constexpr (nstages > 1) {
        // cp_async is a straight global->shared DMA with no ALU on the way, so it cannot
        // dequantize. The launcher only selects the turbo kernel when nstages <= 1.
        static_assert(!turbo_KV, "turbo KV is incompatible with the cp_async multi-stage pipeline");
    ...
    for (int k0_start = (DKQ/2-1) - (DKQ/2-1) % nbatch_K2; k0_start >= 0; k0_start -= nbatch_K2) {
        ...
        if constexpr (nstages <= 1) {
            ...
            if constexpr (turbo_KV == FATTN_MMA_TURBO4) {
                ...
                // half2-sized units (D=256 -> 2 blocks x 68 B = 136 B = 34 half2).
                ...
                flash_attn_ext_turbo5p_load_tile<stride_tile_K, nwarps, nbatch_fa, oob_check>
                    ((const char *) (K_h2 + int64_t(k_VKQ_0)*stride_K), tile_K, k0_diff,
                     stride_K*(int)sizeof(half2), turbo_e0 + k0_start*2, k_VKQ_sup);
            } else if constexpr (turbo_KV == FATTN_MMA_TURBO5P512) {
                // [TAG_TURBO5P512] 336-byte block, also divisible by sizeof(half2).
                static_assert(nbatch_K2 % 16 == 0, "turbo5p512 tile loader needs 32-element chunks");
                flash_attn_ext_turbo5p512_load_tile<stride_tile_K, nwarps, nbatch_fa, oob_check>
                    ((const char *) (K_h2 + int64_t(k_VKQ_0)*stride_K), tile_K, k0_diff,
                     stride_K*(int)sizeof(half2), turbo_e0 + k0_start*2, k_VKQ_sup);
            } else {
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        if (!last_iter) {
            if (kv_pos) {   // [TAG_FA_POS_MASK]
                flash_attn_ext_f16_gen_mask<ncols1, nwarps, nbatch_fa, oob_check>
                    (kv_pos + k_VKQ_0 + nbatch_fa, q_pos, fattn_seq_at(kv_seq, k_VKQ_0 + nbatch_fa), q_seq,
                     tile_mask, k_VKQ_sup, jt*ncols1, ne01);
            } else if (ncols2 > 1 || mask_h) {
                flash_attn_ext_f16_load_mask<ncols1, nwarps, nbatch_fa, use_cp_async, oob_check, use_sparse>
        ...
        if constexpr (nstages <= 1) {
            ...
            if (!V_is_K_view || i0_stop > 2*nbatch_K2) {
                ...
                if constexpr (turbo_KV == FATTN_MMA_TURBO4) {
                    ...
                    static_assert(nbatch_V2 % 16 == 0, "turbo5p512 tile loader needs 32-element chunks");
                    flash_attn_ext_turbo5p512_load_tile<stride_tile_V, nwarps, nbatch_fa, oob_check>
                        ((const char *) (V_h2 + int64_t(k_VKQ_0)*stride_V), tile_V, i0_diff/2,
                         stride_V*(int)sizeof(half2), turbo_e0 + i0_start, k_VKQ_sup);
                } else {
                    flash_attn_ext_f16_load_tile<stride_tile_V, swz, nwarps, nbatch_fa, use_cp_async, oob_check, use_sparse>
...
#else
    GGML_UNUSED_VARS(Q_f2, K_h2, V_h2, mask_h, indices, kv_pos, q_pos, kv_seq, q_seq, dstk, dstk_fixup,
...
template<int DKQ, int DV, int ncols1, int ncols2, int nwarps, bool use_logit_softcap, bool V_is_K_view, int turbo_KV, bool use_sparse, bool needs_fixup, bool is_fixup>
static __device__ __forceinline__ void flash_attn_ext_f16_process_tile(
    ...
    constexpr int stride_tile_Q = DKQ/2     + 4;
    constexpr bool swz = turbo_KV == FATTN_MMA_TURBO_NONE && ggml_cuda_fattn_mma_get_swizzled(DKQ, DV, ncols1, ncols2); // [TAG_SYNC_TURBO_NOSWZ]
    constexpr int stride_tile_K = ggml_cuda_fattn_mma_get_stride_tile(nbatch_K2, swz);
    constexpr int stride_tile_V = V_is_K_view ? stride_tile_K : ggml_cuda_fattn_mma_get_stride_tile(nbatch_V2, swz);
...
#pragma unroll
    ...
    if constexpr (nstages > 1) {
        ...
        if (kv_pos) {   // [TAG_FA_POS_MASK]
            flash_attn_ext_f16_gen_mask<ncols1, nwarps, nbatch_fa, oob_check>
                (kv_pos + kb0*nbatch_fa, q_pos, fattn_seq_at(kv_seq, kb0*nbatch_fa), q_seq, tile_mask, k_VKQ_sup, jt*ncols1, ne01);
        } else if (ncols2 > 1 || mask_h) {
            flash_attn_ext_f16_load_mask<ncols1, nwarps, nbatch_fa, use_cp_async, oob_check, use_sparse>
    ...
    if constexpr (ncols2 == 1 || use_sparse) {
        ...
        for (; kb0 < kb0_stop-1; ++kb0) {
            ...
            flash_attn_ext_f16_iter
                <DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, turbo_KV, use_sparse, needs_fixup, is_fixup, last_iter, oob_check,
                 T_A_KQ, T_B_KQ, T_C_KQ, T_A_VKQ, T_B_VKQ, T_C_VKQ>
                (Q_f2, K_h2, V_h2, mask_h, indices, kv_pos, q_pos, kv_seq, q_seq, dstk, dstk_fixup, scale, slope, logit_softcap,
...
#else
    GGML_UNUSED_VARS(Q_f2, K_h2, V_h2, mask_h, indices, kv_pos, q_pos, kv_seq, q_seq, sinks_f, dstk, dstk_fixup,
...
template<int DKQ, int DV, int ncols1, int ncols2, bool use_logit_softcap, bool V_is_K_view, int turbo_KV, bool use_sparse>
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        if (block_owns_tile_start) {
            ...
            flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, nwarps, use_logit_softcap, V_is_K_view, turbo_KV, use_sparse, needs_fixup, is_fixup>
                (Q_f2, K_h2, V_h2, mask_h, indices, kv_pos, q_pos, pos_seq.kv, pos_seq.q, sinks_f, dstk, dst_meta, scale, slope, logit_softcap,
...
void ggml_cuda_flash_attn_ext_mma_f16_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // [TAG_TURBO_MMA_NATIVE] decided here because it changes the shared-memory sizing below.
    const ggml_tensor * K_src = dst->src[1];
    const ggml_tensor * V_src = dst->src[2];
    // Default ON. Measured on Qwen3.8-27B-UD-Q4_K_XL at ~120K with DFlash2 n7:
    ...
    // the kernel processes. Native reads instead re-dequantize on EVERY tile load. So the
    // two win in opposite regimes, measured on Qwen3.8-27B-UD-Q4_K_XL:
    //
    //   prefill, Q=512  (wide)  pp512@d245760  conversion 621.55  native 362.01  (-42%)
    //   spec,    Q=8  (narrow)  tg@~120K       conversion  72.28  native  82.86  (+15%)
    //
    ...
    const int turbo_mode = !turbo_native ? FATTN_MMA_TURBO_NONE :
        (K_src->type == GGML_TYPE_TURBO4_0      ? FATTN_MMA_TURBO4     :
         K_src->type == GGML_TYPE_TURBO5P_0     ? FATTN_MMA_TURBO5P    :
         K_src->type == GGML_TYPE_TURBO5P512_0  ? FATTN_MMA_TURBO5P512 : FATTN_MMA_TURBO4P);
    ...
    // [TAG_SYNC_TURBO_NOSWZ] Only the f16 kernel takes upstream's XOR swizzle. The turbo tile loaders
    // write plain rows at i*stride_tile + k*h2, so turbo keeps the unswizzled +4-padded tiles.
    const bool swizzled     = !turbo_ok && ggml_cuda_fattn_mma_get_swizzled(DKQ, DV, ncols1, ncols2, cc);
    ...
    // Read the turbo4 cache directly in the tile loader instead of having
    // fattn-common.cuh materialise the WHOLE cache as F16 first (to_fp16 over
    // ggml_nelements(K), once per layer per token). Measured at d131072: making that
    // conversion free lifts this path 35.51 -> 49.17 t/s, and native reads also shrink
    // the tile loads themselves ~4x on top. Opt-in via TURBO_MMA_NATIVE=1 while new.
    fattn_kernel_t fattn_kernel;
    bool use_sparse = false;
    if (logit_softcap == 0.0f) {
        constexpr bool use_logit_softcap = false;
        // [TAG_SYNC_MMA_SPARSE] Turbo kernels are only instantiated with use_sparse = false: upstream's
        // sparse gather lives in the f16 tile loader. It is also kept off with the positional mask
    ...
    // to allocate or fill the F16 scratch at all.
    // [TAG_4C_POSMASK_MS] pos_ms_ok: gen_mask reads the sequence sets of a multi-sequence positional mask
    launch_fattn<DV, ncols1, ncols2>
        (ctx, dst, fattn_kernel, nwarps, nbytes_shared_total, nbatch_fa, !turbo_ok, !turbo_ok, true, use_sparse, warp_size_host, true);
}
