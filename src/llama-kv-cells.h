...
public:
    ...
    // [TAG_SYNC_KV_CELLS] upstream's seq_pos[s] now holds one (pos, cell) pair per cell of s, so its size() is the
    // same count; the counter stays and is cross-checked against it under TURBOT_TEST_TIER or LLAMA_TURBOT_DEBUG
    uint32_t seq_n_cells(llama_seq_id seq_id) const {
        assert(seq_id >= 0);
        assert(seq_id < LLAMA_MAX_SEQ);

        if (seq_n_check()) {
            GGML_ASSERT(seq_n[seq_id] == seq_pos[seq_id].size());
        }
    ...
    // [TAG_SYNC_KV_CELLS] alias of upstream's seq_get_all()
    const seq_set_t & seq_bits(uint32_t i) const {
        return seq_get_all(i);
    }
...
private:
    ...
    // [TAG_SYNC_KV_CELLS] seq_n_cells() cross-checks seq_n against seq_pos: always in test-turbot (TURBOT_TEST_TIER),
    // otherwise when LLAMA_TURBOT_DEBUG is set to a non-zero value
    static bool seq_n_check() {
#ifdef TURBOT_TEST_TIER
        return true;
#else
        static const bool on = [] {
            const char * e = getenv("LLAMA_TURBOT_DEBUG");
