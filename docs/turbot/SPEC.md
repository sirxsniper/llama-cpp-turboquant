...
9. **Selection.**
   ...
   - Plan from `--kv-tier-plan <file>` or env `LLAMA_TURBOT_PLAN`. Refuse clearly if there is none.
...
2. `LLAMA_TURBOT_PLAN`
3. Otherwise refuse: `"turbot KV cache needs a plan: pass --kv-tier-plan <file> or set LLAMA_TURBOT_PLAN"`.
...
| env | `LLAMA_TURBOT`, `LLAMA_TURBOT_PLAN`, `LLAMA_TURBOT_DEBUG`, `LLAMA_TURBOT_FAIL_UBATCH`, `LLAMA_TURBOT_ATTN_ROT`, `LLAMA_ARG_KV_TIER_PLAN` | D | E |
