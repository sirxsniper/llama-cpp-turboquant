...
struct batch_builder {
    ...
    batch_builder(
            ...
            size_t n_tokens_max = 64) // [TAG_LAYER_INP_SCATTER] the scatter layouts build batches of up to 1024 tokens
        : n_embd(n_embd),
          b(n_tokens_max, n_embd, n_embd_inp_enc > 0 ? n_embd_inp_enc : n_embd, n_seq_max, mem, n_vocab, n_pos_per_embd) {}
...
static void test_layer_inp_scatter(testing & t) {
    ...
    auto check_layout = [&](testing & t, std::initializer_list<int> n_tokens, uint32_t n_ubatch, uint32_t keep_tail,
        ...
        // the upstream builder holds 64 tokens by default; these layouts need the whole view
        batch_builder bb(/*n_embd*/ 1, /*mem*/ nullptr, /*n_seq_max*/ 4, /*n_pos_per_embd*/ 1, /*n_vocab*/ 0,
                         /*n_embd_inp_enc*/ 0, /*n_tokens_max*/ n_total);
        llama_seq_id s = 0;
        for (int n : n_tokens) {
            for (int i = 0; i < n; ++i) {
                bb.add(i, {s}, i == n - 1);
        ...
        const int32_t n_batch = (int32_t) bb.b.tokens.size();
        ...
        for (llama_ubatch ub = ba.split_equal(n_ubatch, false, keep_tail); ub.n_tokens > 0;
            ...
            for (uint32_t r = 0; r < ub.n_tokens; ++r) {
                // batch_builder::row() stores embd = 100 * batch index + k (k = 0 here), so the row tells us where it came from
